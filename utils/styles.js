...
async function initGtk(mode, reset = false) {
  const targetDir = `${GLib.get_home_dir()}/.themes/colors.css`;
  const colors = theme[mode];
  const defineColor = (key, value, alpha = 1.0) =>
...
function shorthand(value, length) {
...
function applyOffsets(short, element) {
...
export default async function () {
  options.handler(["theme", "bar.position"], async () => {
    const mode = options.theme.mode.get();
    await initGtk(mode, true).catch(console.error);
    await initScss(mode).catch(console.error);
  });

  await initGtk(options.theme.mode.get()).catch(console.error);
  await initScss(options.theme.mode.get()).catch(console.error);
}
